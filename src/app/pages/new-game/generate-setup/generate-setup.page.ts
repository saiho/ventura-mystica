...
import { flatten, groupBy, sample, sampleSize, shuffle, take } from 'lodash-es';
...
export class GenerateSetupPage {
  ...
  constructor(
    ...
    const groupedByTerrain = sampleSize(groupBy(this.setup.factions, 'terrain'), this.setup.numPlayers);
    if (this.setup.factionPickMode === FactionPickMode.bid) {
      // Pick randomly one faction of each terrain
      this.pickedFactionsBid = groupedByTerrain.map(l => sample(l)!);
      this.pickedFactionsTwoAlternatives = null;
    } else {
      // Pick randomly two factions of each terrain
      this.pickedFactionsTwoAlternatives = groupedByTerrain.map(l => sampleSize(l, 2));
      this.pickedFactionsBid = null;
    }

    // Pick bonus cards randomly
    this.pickedBonusCards = sampleSize(this.setup.bonusCards, this.setup.numPlayers + 3);
    ...
    do {
      this.pickedScoringTiles = sampleSize(this.setup.scoringTiles, TOTAL_ROUNDS);
      maxIter--;
    } while (
    ...
    this.pickedExtraFinalScoringTile = sample(this.setup.extraFinalScoringTiles)!;

    // Pick game board randomly
    this.pickedGameBoard = sample(this.setup.gameBoards)!;

    // Pick artifacts
    switch (this.setup.artifacts) {
      case ArtifactPickMode.asNumPlayersLessOne:
        this.pickedArtifacts = sampleSize(ARTIFACTS_ALL, this.setup.numPlayers - 1);
        this.pickedArtifactsTwoAlternatives = null;
        break;
      case ArtifactPickMode.asNumPlayers:
        this.pickedArtifacts = sampleSize(ARTIFACTS_ALL, this.setup.numPlayers);
        this.pickedArtifactsTwoAlternatives = null;
        break;
      case ArtifactPickMode.assignToFaction:
        if (this.setup.factionPickMode === FactionPickMode.bid) {
          this.pickedArtifacts = sampleSize(ARTIFACTS_ALL, this.pickedFactionsBid!.length);
          this.pickedArtifactsTwoAlternatives = null;
        } else {
          this.pickedArtifacts = null;
          const allPickedArtifacts: Artifact[] = sampleSize(ARTIFACTS_ALL, flatten(this.pickedFactionsTwoAlternatives).length);
    ...
    this.playerOrder = shuffle(take([1, 2, 3, 4, 5], this.setup.numPlayers));
  }
